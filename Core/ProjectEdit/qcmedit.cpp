...
QString QcmEdit::nameOf(QString const& path){
    QString rPath(path);
    QString name;
    while(rPath.back() != "/" && rPath.back() != "\\"){
        name.insert(0, rPath.back());
        rPath.remove(rPath.size()-1, 1);
    }
    return name;
}
        ...
        while(getline(open, ligne)){
            ...
            case 'q':
                ...
                while(getline(open, ligne) && ligne != "<!ques>"){
                    ...
                    indexA++;
                    choices.push_back(Choice(name, indexA, ok));
                }

                // Et on rajoute une question au vector :
                indexQ++;
                questions.push_back(Question(nullptr, qname, choices.size(), indexQ));
                // On attribut les choix trouvés
                questions.back().setChoices(choices);
                break;
            }
        }
        m_projects.push_back(Project(empla, Pname));
...
void QcmEdit::nouveau(){
    ...
    a.exec();
    if(a.ok()){
        m_projects.push_back(Project(a.empla(), a.name()));
...
void QcmEdit::on_actionFermer_triggered(){
    if(m_Gprojects.count() > 0 && m_projects.size() > 0){
        ...
        for(int i(0); i<m_Gprojects.currentIndex(); i++, it++);
        m_projects.erase(it);
...
bool QcmEdit::save(Project const& project){
